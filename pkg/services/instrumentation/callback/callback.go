			...
			defer func() {
				...
				if v := recover(); v != nil {
					if utils.OnPanicFunc != nil {
						...
						utils.OnPanicFunc(heimdallErrors.NewUnknownError(v))
					}
...
func reportBreakpoint(bpInstance *probes.BreakpointInstance, bpInfo *BreakpointInfo, goid int) {
			...
			return func() {
				defer func() {
					if r := recover(); r != nil {
						fmt.Println(r)
						fmt.Println("stacktrace from error: \n" + string(debug.Stack()))
						// locations[i].SetError(heimdallErrors.NewUnknownError(r))
					}
				}()
				...
				locations[i].GetProbe().Execute(collectionService)
			}
		}(i))
	}
	wg.Wait()
