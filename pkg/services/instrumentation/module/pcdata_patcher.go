...
type PCDataPatcher struct {
	...
	isPatched      bool
	instSizeReader func(pc uintptr) (uintptr, heimdallErrors.HeimdallError)
}

func verifyOffsetMappings(offsetMappings []AddressMapping) heimdallErrors.HeimdallError {
	numLastMappingsToCheck := 2
	if len(offsetMappings) < numLastMappingsToCheck {
		return heimdallErrors.NewIllegalAddressMappings()
	}
...
func NewPCDataPatcher(newFuncEntry uintptr, offsetMappings []AddressMapping, isPatched bool, instSizeReader func(uintptr) (uintptr, heimdallErrors.HeimdallError)) (*PCDataPatcher, error) {
