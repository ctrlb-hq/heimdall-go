...
func (m *moduleDataPatcher) buildPCData() error {
	...
	for tableIndex := 0; tableIndex < numTablesInPatched; tableIndex++ {
		...
		if _, ok := os.LookupEnv("ATLAS_DEV_DEBUG"); ok {
...
func verifySingleCallbackPCDatas(newFunc FuncInfo, cbInfo callbackVerificationInfo) heimdallErrors.HeimdallError {
	...
	for newPC := cbInfo.startAddress; newPC < cbInfo.endAddress; {
		for pcdataIdx, origValue := range cbInfo.origPCDataValue {
			...
			if newPC == cbInfo.startAddress || pcdataIdx != _PCDATA_UnsafePoint {
				if origValue != newValue {
					return heimdallErrors.NewPCDataVerificationFailed(uint32(pcdataIdx), origValue, cbInfo.origPC, newValue, newPC)
				}
			} else {

				if newValue != _PCDATA_UnsafePointUnsafe {
					return heimdallErrors.NewPCDataAsyncUnsafePointVerificationFailed(newValue, newPC)
				}
			}
		}
		newFile, newLine := funcline1(newFunc, newPC, false)
		if cbInfo.origFile != newFile {
			return heimdallErrors.NewPCFileVerificationFailed(cbInfo.origFile, cbInfo.origPC, newFile, newPC)
		}
		if cbInfo.origLine != newLine {
			return heimdallErrors.NewPCLineVerificationFailed(cbInfo.origLine, cbInfo.origPC, newLine, newPC)
		}
		...
		for _, pcspEntry := range expectedPCSP {
			if newPC < pcspEntry.Offset {
				...
				if pcspEntry.Value != newSP {
					return heimdallErrors.NewPCSPInPatchedVerificationFailed(cbInfo.origPCSP, cbInfo.origPC, pcspEntry.Value, newSP, newPC)
				}
				foundPCSP = true
				break
			}
		...
		if !foundPCSP {
			return heimdallErrors.NewPCSPVerificationFailedMissingEntry(cbInfo.origPCSP, cbInfo.origPC, newPC)
		}
...
func verifyCallbacksPCDatas(nPCData uint32, f1, f2 FuncInfo, addressMappings []AddressMapping) heimdallErrors.HeimdallError {
...
func verifyNonCallbackPCDatas(nPCData uint32, f1, f2 FuncInfo, addressMappings []AddressMapping) heimdallErrors.HeimdallError {
	for _, mapping := range addressMappings {
		...
		for i := uint32(0); i < nPCData; i++ {
			...
			if value1 != value2 {
				return heimdallErrors.NewPCDataVerificationFailed(i, value1, pc1, value2, pc2)
			}
		}
		...
		if sp1 != sp2 {
			return heimdallErrors.NewPCSPVerificationFailed(sp1, pc1, sp2, pc2)
		}

		file1, line1 := funcline1(f1, pc1, false)
		file2, line2 := funcline1(f2, pc2, false)
		if file1 != file2 {
			return heimdallErrors.NewPCFileVerificationFailed(file1, pc1, file2, pc2)
		}
		if line1 != line2 {
			return heimdallErrors.NewPCLineVerificationFailed(line1, pc1, line2, pc2)
		}
	}
	return nil
}

func verifyPCDatas(f1, f2 FuncInfo, addressMappings []AddressMapping) heimdallErrors.HeimdallError {
	if f1.npcdata != f2.npcdata {

		if f1.npcdata > _PCDATA_UnsafePoint || f2.npcdata != _PCDATA_UnsafePoint+1 {

			return heimdallErrors.NewDifferentNPCData(uint32(f1.npcdata), uint32(f2.npcdata))
		}
...
func verifyFuncDatas(f1, f2 FuncInfo) heimdallErrors.HeimdallError {
	if f1.nfuncdata != f2.nfuncdata {
		return heimdallErrors.NewDifferentNFuncData(f1.nfuncdata, f2.nfuncdata)
	}
	...
	for i := 0; i < int(f1.nfuncdata); i++ {
		...
		if value1 != value2 {
			return heimdallErrors.NewFuncDataVerificationFailed(i, uintptr(value1), uintptr(value2))
		}
	}
...
func (m *moduleDataPatcher) verifyModule(module *moduledata) (err heimdallErrors.HeimdallError) {
	defer func() {
		if r := recover(); r != nil {
			err = heimdallErrors.NewModuleVerificationFailed(r)
		}
	}()
...
func PatchModuleData(addressMappings []AddressMapping, offsetMappings []AddressMapping, stateID int) error {
	...
	moduleName := fmt.Sprintf("ATLAS-%s[%x]-%d", funcName, function.getEntry(), stateID)
