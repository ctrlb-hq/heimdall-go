...
func (b *Builder) AddInstructions(p *Instruction, instructions ...*Instruction) heimdallErrors.HeimdallError {
	insts := []*Instruction{p}
	...
	for _, inst := range insts {
		if inst.label != "" {
			if _, ok := b.labels[inst.label]; ok {
				return heimdallErrors.NewLabelAlreadyExists(inst.label)
			}
...
func (b *Builder) fixup() heimdallErrors.HeimdallError {
	for inst := b.first; inst != nil; inst = inst.link {
		...
		if inst.placeholderFor != nil {
			if b.arch.Arch.Name == "arm64" && len(inst.placeholderFor)%4 != 0 {
				return heimdallErrors.NewInvalidBytes(inst.placeholderFor)
			}
		...
		if inst.jumpDestLabel != "" {
			...
			if !ok {
				return heimdallErrors.NewInvalidJumpDest(inst.jumpDestLabel)
			}
...
func (b *Builder) Assemble() ([]byte, heimdallErrors.HeimdallError) {
	err := b.fixup()
	if err != nil {
		return nil, err
	}
	...
	err = func() (err heimdallErrors.HeimdallError) {
		defer func() {
			if r := recover(); r != nil {
				err = heimdallErrors.NewFailedToAssemble(r)
			}
		}()
