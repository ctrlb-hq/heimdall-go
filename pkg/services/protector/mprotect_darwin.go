...
func GetMemoryProtection(addr uint64, size uint64) (int, heimdallErrors.HeimdallError) {
	...
	if prot == -1 {
		return 0, heimdallErrors.NewFailedToGetCurrentMemoryProtection(addr, size)
	}
	return prot, nil
}
...
func ChangeMemoryProtection(start uintptr, end uintptr, prot int) heimdallErrors.HeimdallError {
	...
	if ret != 0 {
		heimdallErrors.NewMprotectFailed(start, int(end-start), prot, fmt.Sprintf("mprotect failed: %d", ret))
	}
	return nil
}
