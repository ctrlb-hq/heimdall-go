...
func (r *regTracker) ResolveRegsUsed(variableType godwarf.Type, pieces []op.Piece) ([]assembler.Reg, heimdallErrors.HeimdallError) {
	if len(pieces) == 0 {
		return nil, nil
	}
	...
	for _, piece := range pointerPieces {
		...
		if !ok {
			return nil, heimdallErrors.NewInvalidDwarfRegister(piece.Val)
		}
