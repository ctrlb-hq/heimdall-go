...
func Decode(startPC uintptr, endPC uintptr, skipUnknown bool) ([]*Instruction, heimdallErrors.HeimdallError) {
	...
	for offset < funcLen {
		...
		if err != nil {
			...
			if err.Error() == "unknown instruction" && skipUnknown {
				...
				return nil, heimdallErrors.NewFailedToDecode(funcAsm[offset:], err)
			}
		}
