...
func newMapsActions() (m *mapsActions, err heimdallErrors.HeimdallError) {
	m = &mapsActions{}
...
func (m *mapsActions) buildParser() (*peg.Parser, heimdallErrors.HeimdallError) {
    ...
    `)
	if nil != err {
		return nil, heimdallErrors.NewArithmeticPathException(err)
	}
...
func (m *mapsActions) Parse(path string) (node, heimdallErrors.HeimdallError) {
	operations, err := m.parser.ParseAndGetValue(path, nil)
	if err != nil {
		return nil, heimdallErrors.NewHeimdallInvalidArithmeticPathException(path, err)
	}
...
func (m *mapsActions) makeComparisonExpression(values *peg.Values, _ peg.Any) (peg.Any, error) {
	if values.Len() == 2 {
		return nil, heimdallErrors.NewHeimdallInvalidArithmeticPathException(values.SS, nil)
	}
...
func (m *mapsActions) makeNumber(values *peg.Values, _ peg.Any) (peg.Any, error) {
	...
	if err != nil {
		...
		return nil, heimdallErrors.NewHeimdallInvalidArithmeticPathException(msg, err)
	}
