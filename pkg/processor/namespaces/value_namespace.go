...
func (v *ValueNamespace) CallMethod(name string, args string) (n Namespace, err heimdallErrors.HeimdallError) {
	defer func() {
		if recover() != nil {
	...
	case "size":
		...
		if size == nil {
			return nil, heimdallErrors.NewObjectHasNoSizeException(v.value.Type().String())
		}
		return size, nil
...
func (v *ValueNamespace) ReadAttribute(name string) (n Namespace, err heimdallErrors.HeimdallError) {
	value := v.value
...
func (v *ValueNamespace) WriteAttribute(_ string, _ Namespace) heimdallErrors.HeimdallError {
	return heimdallErrors.NewNotImplemented()
}

func (v *ValueNamespace) ReadKey(key interface{}) (Namespace, heimdallErrors.HeimdallError) {
	value := v.value
