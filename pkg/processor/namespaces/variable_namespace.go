...
func (v *VariableNamespace) CallMethod(name string, args string) (Namespace, heimdallErrors.HeimdallError) {
	switch name {
	case "type":
	...
	case "size":
		...
		if size == nil {
			return nil, heimdallErrors.NewObjectHasNoSizeException(v.GetObject())
		}
		return size, nil
	case "depth":
		maxDepth, err := strconv.Atoi(args)
		if err != nil {
			return nil, heimdallErrors.NewHeimdallInvalidMethodArguments("depth()", args)
		}
	...
	case "width":
		...
		if err != nil {
			return nil, heimdallErrors.NewHeimdallInvalidMethodArguments("width()", args)
		}
	...
	case "collection_dump":
		...
		if err != nil {
			return nil, heimdallErrors.NewHeimdallInvalidMethodArguments("collection_dump()", args)
		}
	...
	case "string":
		...
		if err != nil {
			return nil, heimdallErrors.NewHeimdallInvalidMethodArguments("string()", args)
		}
	...
	case "limit":
		...
		return nil, heimdallErrors.NewHeimdallInvalidMethodArguments("limit()", args)
...
func (v *VariableNamespace) ReadAttribute(name string) (Namespace, heimdallErrors.HeimdallError) {
	value := v.Obj
...
func (v *VariableNamespace) WriteAttribute(_ string, _ Namespace) heimdallErrors.HeimdallError {
	return heimdallErrors.NewNotImplemented()
}

func (v *VariableNamespace) readKeyFromArray(key int) (Namespace, heimdallErrors.HeimdallError) {
	name := v.name + "[" + strconv.Itoa(key) + "]"

	if int(v.Obj.Len) <= key {
		return nil, heimdallErrors.NewAgentKeyNotFoundException(v.name, key, nil)
	}
	...
	if len(v.Obj.Children) > key {
		...
		if err == nil {
			return nil, heimdallErrors.NewAgentKeyNotFoundException(v.name, key, err)
		}
	}
...
func (v *VariableNamespace) readKeyFromMap(key string) (Namespace, heimdallErrors.HeimdallError) {
	name := v.name + "[\"" + key + "\"]"
...
func (v *VariableNamespace) readKeyFromStruct(key string) (Namespace, heimdallErrors.HeimdallError) {
	name := v.name + "." + key
...
func (v *VariableNamespace) ReadKey(key interface{}) (Namespace, heimdallErrors.HeimdallError) {
	switch v.Obj.Kind {
	...
	case reflect.Interface, reflect.Ptr:
		...
		if v.Obj.Kind == obj.Obj.Kind {
			return nil, heimdallErrors.NewInvalidInterfaceVariable(key)
		}
