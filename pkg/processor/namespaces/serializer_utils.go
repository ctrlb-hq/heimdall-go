...
type Namespace interface {
	CallMethod(name string, args string) (Namespace, heimdallErrors.HeimdallError)

	WriteAttribute(name string, value Namespace) heimdallErrors.HeimdallError
	ReadAttribute(name string) (Namespace, heimdallErrors.HeimdallError)

	ReadKey(key interface{}) (Namespace, heimdallErrors.HeimdallError)
	GetObject() interface{}
...
type Serializer interface {
	...
	dumpNil()
	dumpUnsupported()
	dumpHeimdallError(err heimdallErrors.HeimdallError)
...
func dumpValue(s Serializer, value reflect.Value, config config.ObjectDumpConfig) {
	...
	case reflect.TypeOf(time.Time{}):
		...
		return
	case reflect.TypeOf(fmt.Errorf("")), reflect.TypeOf(&heimdallErrors.HeimdallErrorImpl{}):
...
func dumpError(s Serializer, err error) {
	if r, ok := err.(heimdallErrors.HeimdallError); ok {
		s.dumpHeimdallError(r)
		return
	}
...
func dumpErrorValue(s Serializer, value reflect.Value) {
	if value.Type() == reflect.TypeOf(&heimdallErrors.HeimdallErrorImpl{}) {
		value = value.Elem()
	}

	if value.Type() == reflect.TypeOf(heimdallErrors.HeimdallErrorImpl{}) {
		err := &heimdallErrors.HeimdallErrorImpl{
